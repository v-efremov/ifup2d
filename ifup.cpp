...
long double testimage[]={1.0000000025607425, 1.0000034575202832, 1.0002296211519204,
1.0018904349931375, 1.0023142471714943, 1.0004992417451533,
      1.000014508934933, 1.0000000180353237, 1.,
        1.0000034575202827, 1.0016632502727547, 1.039626961378202, 
          1.159235825072223, 1.183710906381325, 1.0671559030462219, 
          1.0045356086362005, 1.0000233550591353, 
          1.0000000008312357,
        1.00022962115192, 1.039626961378202, 1.3916518416991812, 
          1.7760000905949596, 1.8187883375855227, 1.5188570417426293,
           1.0913768176133072, 1.0014071283674104, 
          1.00000037564291,
        1.0018904349931375, 1.1592358250722232, 1.7760000905949596, 
          1.991071162368386, 1.9963394706910373, 1.8923778295707876, 
          1.3199030875159177, 1.0110094784114048, 
          1.000008783990875,
        1.0023142471714943, 1.183710906381325, 1.8187883375855232, 
          1.9963394706910373, 1.9993574803514214, 1.935098728892264, 
          1.3869059162726183, 1.0158564132099182, 
          1.000014927443585,
        1.0004992417451533, 1.067155903046222, 1.5188570417426293, 
          1.8923778295707876, 1.935098728892264, 1.7106066847410506, 
          1.176184482317732, 1.0038892399418635, 
          1.0000017940912411,
        1.0000145089349328, 1.0045356086362007, 1.091376817613307, 
          1.3199030875159177, 1.3869059162726183, 1.1761844823177319,
           1.0188013458363614, 1.000163833123728, 
          1.0000000151678552,
        1.0000000180353237, 1.0000233550591353, 1.0014071283674104, 
          1.0110094784114048, 1.0158564132099184, 1.0038892399418635,
           1.0001638331237281, 1.000000467881383, 1.,
        0.9999999999999999, 1.0000000008312357, 1.0000003756429099, 
          1.000008783990875, 1.000014927443585, 1.0000017940912411, 
          1.0000000151678552, 1., 1.
        };
...
//        ridots[(i+(Mx-Nx))*Nx+j]=double(im0.getrpixel(i,j))/255+1;
  }
...
long double ifupf1 (atomfunc upn1, long double x, long double y, dword Nx, long double* DDDX)
  {
//    cout << "in ifupf1 " << ",upn(2,4)=" << upn1.upn(2,4)<<"\n";
    ...
    for(i=0;i<Mx;i++)
      {
        ...
        for(j=0;j<Mx;j++)
          {
             u=upn1.upn(y+cx-double(j)*Hx,NUpx);
//             cout <<"y="<<y<<",cx="<<cx<<",j="<<j<<",Hx="<<Hx<<",NUpx="<<NUpx<<",u="<<u;
             sy=sy+DDDX[j*Mx+i]*u;
//             cout <<",DDDX[j,i]="<<DDDX[j*Mx+i]<<",upn("<<y+cx-double(j)*Hx<<","<<NUpx<<")="<<u<<",sy="<<sy<<"\n";
          }
        sx=sx+sy*upn1.upn(x+cx-i*Hx,NUpx);
      }
    return sx;
  }
...
void showf1(atomfunc upn1, dword Nx, long double* DDD2)
  {
     cout << "in showf1 " << ",upn(2,4)=" << upn1.upn(2,4)<<"\n";
     for(int i=0;i<Nx;i++)
       {
...
//основные структуры данных
  ...
  atomfunc upn1(Nx,10000);  //базисная функция
...
//матрица атомарных функций для построения атомарного многчлена
  cout << "in main 1" << ",upn(2,4)=" << upn1.upn(2,4)<<"\n";
  upsxinit(upn1,Nx,upsx); //в upsx матрица A для СЛАУ
  cout << "in main 2" << ",upn(2,4)=" << upn1.upn(2,4)<<"\n";
  showupsx(Nx,upsx);
  cout << "in main 3" << ",upn(2,4)=" << upn1.upn(2,4)<<"\n";
  saveupsx(Nx,upsx);
  cout << "in main 4" << ",upn(2,4)=" << upn1.upn(2,4)<<"\n";
  cout<<clock()<<" table upsx of coefficients done;\n";
  ridotsinit(Im0,Nx,ridots);
  cout << "in main 5" << ",upn(2,4)=" << upn1.upn(2,4)<<"\n";
  showidots(Nx,ridots);
  cout << "in main 6" << ",upn(2,4)=" << upn1.upn(2,4)<<"\n";
  cout<<clock()<<" table idots of right parts done;\n";
//построение многочлена решением СЛАУ
  findDDD1(upsx,ridots,Nx,DDD1);
  atomfunc upn2(Nx,10000);  //базисная функция
  cout << "in main 7" << ",upn(2,4)=" << upn2.upn(2,4)<<"\n";
  showDDD1(Nx,DDD1);
  cout << "in main 8" << ",upn(2,4)=" << upn2.upn(2,4)<<"\n";
  cout<<clock()<<" table DDDY done;\n";
  findDDD2(upsx,DDD1,Nx,DDD2);  
  cout << "in main 9" << ",upn(2,4)=" << upn2.upn(2,4)<<"\n";
  showDDD2(Nx,DDD2);
  cout << "in main 10" << ",upn(2,4)=" << upn2.upn(2,4)<<"\n";
  cout<<clock()<<" table DDDX done;\n";
  showf1(upn2,Nx,DDD2);
  cout<<clock()<<" table f1 done;\n";
  cout << "in main 11" << ",upn(2,4)=" << upn2.upn(2,4)<<"\n";
